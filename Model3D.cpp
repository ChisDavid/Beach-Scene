...
namespace gps {
	...
	glm::vec3 minim(std::vector<gps::Vertex> a,glm::vec3 min)
	{
		
		for (float i = 0; i < a.size(); i++)
		{
			if (a[i].Position.x <min.x)
			{
				min.x = a[i].Position.x;
			}
			if (a[i].Position.y < min.y)
			{
				min.y = a[i].Position.y;
			}
			if (a[i].Position.z < min.z)
			{
				min.z = a[i].Position.z;
			}
		}
		return min;
	}
	...
	//		min = minim(meshes[i].vertices, min);
		}
